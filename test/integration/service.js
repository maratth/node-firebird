  ...
  after((done) => {
    ...
    fs.readdir(path.resolve(__dirname), (err, fileNames) => {
      ...
      for (const name of fileNames) {
        if (
          name.toLowerCase().indexOf(".fdb") > -1 ||
          name.toLowerCase().indexOf(".fbk") > -1
        ) {
    ...
    it("should get all server infos", (done) => {
      Firebird.attach(config, (err, srv) => {
        ...
        srv.getFbserverInfos([], {}, (err, data) => {
          ...
          assert.ok(
            data.dbinfo.nbattachment != null,
            "Db info nb attachment not found"
          );
          assert.ok(
            data.dbinfo.nbdatabase != null,
            "Db info nb database not found"
          );
      ...
      Firebird.attach(config, (err, srv) => {
        ...
        srv.getStats({}, (err, data) => {
          ...
          readStream(data, (result) => {
            assert.ok(
              result.indexOf("Gstat execution time") > -1,
              '"Gstat execution time" text not found'
            );
  ...
  describe("Server properties", () => {
    ...
    const SHUTDOWN = [
      {
        name: "force - multi",
        args: [0, 0, 1],
        test: /Attributes\s*.*? multi-user maintenance/,
      },
      {
        name: "force - single",
        args: [0, 0, 2],
        test: /Attributes\s*.*? single-user maintenance/,
      },
      {
      ...
      {
        name: "attachement - multi",
        args: [2, 0, 1],
        test: /Attributes\s*.*? multi-user maintenance/,
      },
      {
        name: "attachement - single",
        args: [2, 0, 2],
        test: /Attributes\s*.*? single-user maintenance/,
      },
    ];
    ...
    it("should enable reverse space", (done) => {
      testProperty(
        "setReservespace",
        [DATABASE.database, true],
        (data) => !data.indexOf("no reverse") > -1,
        done
      );
    });
      ...
      Firebird.attach(config, (err, srv) => {
        ...
        srv.removeUser(EXPECTED_USER.username, "", (err, data) => {
          ...
          srv.getUsers("", (err, userData) => {
            assert.ok(!err, err);
            const users = userData.fbusers.filter(
              (u) => u.username === EXPECTED_USER.username
            );
  ...
  describe("Backup/Restaure", () => {
    it("should backup", (done) => {
      const BACKUP_OPTS = {
        ...
        files: [{ filename: DATABASE.database.replace(".fdb", "-backup.fbk") }],
      };
      ...
      Firebird.attach(config, (err, srv) => {
        ...
        srv.backup(BACKUP_OPTS, (err, data) => {
          ...
          data.on("end", () => {
            assert.ok(
              fs.existsSync(path.resolve(BACKUP_OPTS.files[0].filename))
            );
            srv.detach(done);
          });
        });
    ...
    it("should restore", (done) => {
      const RESTORE_OPTS = {
        ...
        files: [DATABASE.database.replace(".fdb", "-backup.fbk")],
      };
    ...
    it("should nrestore", (done) => {
      const RESTORE_OPTS = {
        ...
        files: [DATABASE.database.replace(".fdb", "-nbackup.fbk")],
      };
  ...
  describe("Trace", () => {
    ...
    before((done) => {
      Firebird.attach(config, (err, srv) => {
        ...
        srv.getFbserverInfos({ fbversion: true }, {}, (err, data) => {
          ...
          var matches = data.fbversion.match(
            /((\w{2})-(\w)(\d+)\.(\d+)\.(\d+)\.(\d+)(?:-\S+)?) (.+)/
          );
          var serverMajorVersion = parseInt(matches[4]);
          var traceConfigFile =
            serverMajorVersion > 2 ? "fbtrace-3.conf" : "fbtrace-2.conf";
          traceConfig = fs.readFileSync(
            path.resolve(Config.testDir, traceConfigFile),
            { encoding: "utf8" }
          );
    ...
    it("should start trace", (done) => {
      Firebird.attach(config, (err, srv) => {
        ...
        srv.startTrace(
          { configfile: traceConfig, tracename: traceName },
          (err, data) => {
            assert.ok(!err, err);
            assert.ok(data instanceof stream.Readable);
            ...
            var result = "";
            data.on("data", (chunk) => {
              traceId = chunk.substring(
                "Trace session ID ".length,
                chunk.indexOf(" started")
              );
              assert.ok(traceId);
              result += chunk;
      ...
      Firebird.attach(config, (err, srv) => {
        ...
        srv.suspendTrace({ traceid: traceId }, (err, data) => {
          ...
          readStream(data, (result) => {
            assert.equal(
              result.trim(),
              "Trace session ID " + traceId + " paused"
            );
      ...
      Firebird.attach(config, (err, srv) => {
        ...
        srv.resumeTrace({ traceid: traceId }, (err, data) => {
          ...
          readStream(data, (result) => {
            assert.equal(
              result.trim(),
              "Trace session ID " + traceId + " resumed"
            );
      ...
      Firebird.attach(config, (err, srv) => {
        ...
        srv.getTraceList({}, (err, data) => {
          ...
          readStream(data, (result) => {
            ...
            assert.ok(
              result.indexOf("user:  " + config.user.toUpperCase()) > -1
            );
      ...
      Firebird.attach(config, (err, srv) => {
        ...
        srv.stopTrace({ traceid: traceId }, (err, data) => {
          ...
          readStream(data, (result) => {
            assert.equal(
              result.trim(),
              "Trace session ID " + traceId + " stopped"
            );
