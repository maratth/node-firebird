...
const
	...
	DEFAULT_WIRE_CRYPT = WIRE_CRYPT_ENABLED;
...
exports.WIRE_CRYPT_DISABLED = WIRE_CRYPT_DISABLED;
exports.WIRE_CRYPT_ENABLED = WIRE_CRYPT_ENABLED;
exports.WIRE_CRYPT_REQUIRED = WIRE_CRYPT_REQUIRED;
...
exports.Connection.prototype._bind_events = function(host, port, callback) {
    ...
    self._socket.on('data', function (data) {
        ...
        if (self._crypt) {
        	console.log(self.accept.sessionKey.toString(16));
        	var arc4 = new Arc4(self.accept.sessionKey.toString(16));
        	console.log('crypted:' + data.toString('hex'));
        	data = arc4.decrypt(data);
        	console.log('decrypted:' + data.toString('hex'));
		}
    ...
    try {
        ...
        switch (r) {
            ...
            case op_accept_data:
                ...
                const acceptType = data.readInt();
                cnx.accept.protocolMinimumType = acceptType && ptype_mask;
				cnx.accept.compress = (cnx.accept.protocolMinimumType & pflag_compress) !== 0; // TODO Handle zlib compression

                if (cnx.accept.protocolVersion < 0) {
                    cnx.accept.protocolVersion = (cnx.accept.protocolVersion & FB_PROTOCOL_MASK) | FB_PROTOCOL_FLAG;
                }

                console.log('accept.compress:'+cnx.accept.compress);
                if (r === op_cond_accept || r === op_accept_data) {
                    var d = new BlrReader(data.readArray());
                    cnx.accept.pluginName = data.readString(DEFAULT_ENCODING);
                    var is_authenticated = data.readInt();
                    var keys = data.readString(DEFAULT_ENCODING); // keys

                    if (is_authenticated === 0) {
                    	if (cnx.options.pluginName && cnx.options.pluginName !== cnx.accept.pluginName) {
                    		doError(new Error('Server don\'t accept plugin : ' + cnx.options.pluginName + ', but support : ' + accept.pluginName), callback);
						}

                    	if (AUTH_PLUGIN_SRP_LIST.indexOf(cnx.accept.pluginName) !== -1) {
							var crypto = {
								Srp: 'sha1',
								Srp256: 'sha256'
							};
							cnx.accept.srpAlgo = crypto[cnx.accept.pluginName];
							...
							cnx.accept.authData = proof.authData.toString(16);
							cnx.accept.sessionKey = proof.clientSessionKey;
						} else if (cnx.accept.pluginName === AUTH_PLUGIN_LEGACY) {
                            cnx.accept.authData = crypt.crypt(cnx.options.password, LEGACY_AUTH_SALT).substring(2);
                        } else {
                            return cb(new Error('Unknow auth plugin : ' + cnx.accept.pluginName));
                        }
                    } else {
                        cnx.accept.authData = '';
                        cnx.accept.sessionKey = '';
                    }
                }

                console.log(cnx.options.wireCrypt);
                if (true && cnx.accept.sessionKey) {
                	console.log('crypt cnx', data.pos);
                	cnx.cryptConnection(function () {
                		console.log('ici');
                		cb(undefined, cnx.accept);
					});
                	console.log('set channel crypted');
                	cnx._crypt = true;
                	return;
					// return cb(new Error('uncomplete'));
				}

                return cb(undefined, cnx.accept);
				//cb();
			case op_cont_auth:
				var d = new BlrReader(data.readArray());
...
Connection.prototype._queueEvent = function(msg, callback){
	if (callback === undefined) {
		callback = msg;
		msg = undefined;
	}
    ...
    var data = msg ? msg.getData() : self._msg.getData();
    if (self._crypt) {
    	console.log('send crypted');
    	const arc4 = new Arc4(self.accept.sessionKey.toString(16));
    	data = arc4.crypt(data);
	}
...
Connection.prototype._newMsg = function() {
	return new XdrWriter();
}

Connection.prototype._newBlr = function() {
	this._msg = new BlrWriter();
}

Connection.prototype.cryptConnection = function (callback) {
	this._msg = this._newMsg();
	var msg = this._msg;

	msg.addInt(op_crypt);
	msg.addString('Arc4', DEFAULT_ENCODING);
	msg.addString('Symmetric', DEFAULT_ENCODING);
...
Connection.prototype.connect = function (options, callback) {
	...
	var wireCrypt = options.hasOwnProperty('wireCrypt') ? options.wireCrypt : DEFAULT_WIRE_CRYPT;
    var msg = this._msg;
    var blr = this._blr;
	...
	console.log('SendConnect: pluginName=' + pluginName + ' wireCrypt:' + wireCrypt);
	var specificData = '';
	...
	blr.addBytes([CNCT_client_crypt, 4, wireCrypt, 0, 0, 0]); // WireCrypt = Disabled
