...
class SQLParamDouble {
    constructor(value, sqlType = Const.SQL_DOUBLE) {
        this.value = value;
        this.sqlType = sqlType;
    }

    encode(data, batchMode = false) {
        const isFloat = batchMode && this.sqlType === Const.SQL_FLOAT;
        if (this.value != null) {
            isFloat ? data.addFloat(this.value) : data.addDouble(this.value);
        } else {
            isFloat ? data.addFloat(0) : data.addDouble(0);
            data.addInt(1);
        }
    }
...
class SQLParamString {
    constructor(value, sqlType = Const.SQL_TEXT, maxLen = 0) {
        this.value = value;
        this.sqlType = sqlType;
        this.maxLen = maxLen;
    }
    ...
    encode(data, batchMode = false) {
        if (batchMode) {
            ...
            if (this.sqlType === Const.SQL_VARYING) {
                const buf = Buffer.alloc(2 + maxLen, 0);
                if (this.value != null) {
                    const b = Buffer.from(this.value.toString(), Const.DEFAULT_ENCODING);
                    const actualLen = Math.min(b.length, maxLen);
                    buf.writeUInt16BE(actualLen, 0);
                    b.copy(buf, 2, 0, actualLen);
                }
                data.addBuffer(buf);
                data.addAlignment(2 + maxLen);
            } else {
                const buf = Buffer.alloc(maxLen, 0x20); // space-pad for CHAR/NULL
                if (this.value != null) {
                    const b = Buffer.from(this.value.toString(), Const.DEFAULT_ENCODING);
                    b.copy(buf, 0, 0, Math.min(b.length, maxLen));
                }
                data.addBuffer(buf);
                data.addAlignment(maxLen);
            }
            if (this.value == null) {
                data.addInt(1);
            }
...
class SQLParamDate {
    constructor(value, sqlType = Const.SQL_TIMESTAMP) {
        this.value = value;
        this.sqlType = sqlType;
    }
    ...
    encode(data, batchMode = false) {
        if (this.value != null) {
            const ts = this.value.getTime() - this.value.getTimezoneOffset() * MsPerMinute;
            ...
            if (!batchMode) {
                ...
                switch (this.sqlType) {
                    case Const.SQL_TYPE_DATE:
                        data.addInt(date); break;
                    case Const.SQL_TYPE_TIME:
                        data.addUInt(t); break;
                    case Const.SQL_TIME_TZ:
                        data.addUInt(t); data.addInt(0); break; // UTC timezone
                    case Const.SQL_TIME_TZ_EX:
                        data.addUInt(t); data.addInt(0); data.addInt(0); break; // timezone, ext_offset
                    case Const.SQL_TIMESTAMP_TZ:
                        data.addInt(date); data.addUInt(t); data.addInt(0); break; // UTC timezone
                    case Const.SQL_TIMESTAMP_TZ_EX:
                        data.addInt(date); data.addUInt(t); data.addInt(0); data.addInt(0); break; // timezone, ext_offset
                    default: // SQL_TIMESTAMP
                        data.addInt(date); data.addUInt(t);
                }
            }
        } else {
            if (!batchMode) {
                ...
                switch (this.sqlType) {
                    case Const.SQL_TYPE_DATE:
                        data.addInt(0); break;
                    case Const.SQL_TYPE_TIME:
                        data.addUInt(0); break;
                    case Const.SQL_TIME_TZ:
                        data.addUInt(0); data.addInt(0); break;
                    case Const.SQL_TIME_TZ_EX:
                        data.addUInt(0); data.addInt(0); data.addInt(0); break;
                    case Const.SQL_TIMESTAMP_TZ:
                        data.addInt(0); data.addUInt(0); data.addInt(0); break;
                    case Const.SQL_TIMESTAMP_TZ_EX:
                        data.addInt(0); data.addUInt(0); data.addInt(0); data.addInt(0); break;
                    default: // SQL_TIMESTAMP
                        data.addInt(0); data.addUInt(0);
                }
                data.addInt(1); // null indicator
            }
        }
    }
