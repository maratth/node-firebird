...
class Connection {
    ...
    // ---- Batch API (Protocol 16+, Firebird 4.0) ----

    batchCreate(statement, blr, bpb, callback) {
        if (this._isClosed) return this.throwClosed(callback);
        this._pending.push('batchCreate');
        var msg = this._msg;
        msg.pos = 0;
        msg.addInt(Const.op_batch_create);
        msg.addInt(statement.handle);
        msg.addBlr(blr);
        msg.addInt(0); // message type = 0
        msg.addInt(bpb.length);
        msg.addBuffer(bpb);
        msg.addAlignment(bpb.length);
        this._queueEvent(callback);
    }

    batchSendMessages(statement, msgData, msgCount) {
        var msg = this._msg;
        msg.pos = 0;
        msg.addInt(Const.op_batch_msg);
        msg.addInt(statement.handle);
        msg.addInt(msgCount);
        msg.addInt(msgData.length);
        msg.addBuffer(msgData);
        msg.addAlignment(msgData.length);
        // Server sends no response for op_batch_msg; write directly to socket
        this._socket.write(Buffer.from(msg.getData()));
    }

    batchExecute(statement, transaction, callback) {
        if (this._isClosed) return this.throwClosed(callback);
        this._pending.push('batchExecute');
        var msg = this._msg;
        msg.pos = 0;
    ...
    batchRelease(statement, callback) {
        this._pending.push('batchRelease');
        var msg = this._msg;
        msg.pos = 0;
    ...
    executeBatch(transaction, statement, paramsArray, options, callback) {
        if (typeof options === 'function') {
            callback = options;
            options = {};
        }
        ...
        if (!paramsArray || paramsArray.length === 0) {
            return callback(null, { updated: [], errors: [], affectedRows: 0, hasErrors: false });
        }

        // Fall back to sequential execution for pre-V16 protocols
        if (this.accept.protocolVersion < Const.PROTOCOL_VERSION16) {
            return this._executeBatchFallback(transaction, statement, paramsArray, callback);
        }
        ...
        if (!input || input.length === 0) {
            return callback(new Error('Statement has no input parameters'));
        }

        // Normalize each parameter set to an Array
        const normalizedParams = paramsArray.map(params => {
            if (!(params instanceof Array)) {
        ...
        // Prepare BLOB parameters: create and upload all blobs, collect OIDs
        prepareBatchBlobs(self, transaction, normalizedParams, input, function(err, preparedRows) {
            if (err) return callback(err);

            // Compute BLR from input metadata (declared types/lengths for fixed-size messages)
            const blr = new BlrWriter(256);
            CalcBlr(blr, input);

            // Build Batch Parameters Block
            const bpb = buildBPB(options);

            // Encode all rows into a single flat message buffer
            const msgXdr = new XdrWriter(preparedRows.length * Math.max(input.length * 16, 64));
            for (const row of preparedRows) {
                for (let i = 0; i < input.length; i++) {
                    encodeBatchField(msgXdr, row[i], input[i]);
                }
            }
            const msgData = Buffer.from(msgXdr.getData()); // explicit copy for safe async socket writes

            // Create batch (server returns op_response)
            self.batchCreate(statement, blr, bpb, function(err) {
                if (err) return callback(err);

                // Send all parameter messages (no server response)
                self.batchSendMessages(statement, msgData, preparedRows.length);

                // Execute the batch (server returns op_batch_cs_response)
                self.batchExecute(statement, transaction, function(err, result) {
                    // Always release the batch, regardless of execution outcome
                    self.batchRelease(statement, function() {
                        if (err) return callback(err);
    ...
    _executeBatchFallback(transaction, statement, paramsArray, callback) {
        const self = this;
        const updated = [];
        const errors = [];
        let affectedRows = 0;

        function executeRow(i) {
            if (i >= paramsArray.length) {
                return callback(null, { updated, errors, affectedRows, hasErrors: errors.length > 0 });
            }
            ...
            self.executeStatement(transaction, statement, params, function(err) {
                if (err) {
                    updated.push(Const.BATCH_EXECUTE_FAILED);
                    errors.push({ row: i, message: err.message, status: [] });
                } else {
                    updated.push(1);
                    affectedRows++;
                }
    ...
    try {
        ...
        switch (r) {
            ...
            case Const.op_crypt_key_callback:
                ...
                return;
            case Const.op_batch_cs_response: {
                const updateCount = data.readInt();
                const updated = [];
                for (let _i = 0; _i < updateCount; _i++) {
                    updated.push(data.readInt());
                }

                const errorCount = data.readInt();
                const errors = [];
                for (let _e = 0; _e < errorCount; _e++) {
                    const rowIndex = data.readInt();
                    const statusItems = [];
                    let currentItem = {};
                    let statusOp;

                    while ((statusOp = data.readInt()) !== Const.isc_arg_end) {
                        switch (statusOp) {
                            case Const.isc_arg_gds: {
                                const gdsCode = data.readInt();
                                if (gdsCode) {
                                    currentItem = { gdscode: gdsCode };
                                    statusItems.push(currentItem);
                                }
                                break;
                            }
                            ...
                            case Const.isc_arg_sql_state: {
                                ...
                                (currentItem.params = currentItem.params || []).push(str);
                                break;
                            }
                            case Const.isc_arg_number: {
                                const num = data.readInt();
                                (currentItem.params = currentItem.params || []).push(num);
                                if (currentItem.gdscode === Const.isc_sqlerr) {
                                    currentItem.sqlcode = num;
                                }
                                break;
                            }
                            default:
                                data.readInt(); // skip unknown arg
                        }
                    }

                    const message = statusItems.length > 0
                        ? lookupMessages(statusItems)
                        : ('Batch row ' + rowIndex + ' failed');
                    errors.push({ row: rowIndex, status: statusItems, message });
                }

                const affectedRows = updated.reduce((sum, n) => sum + Math.max(0, n), 0);
                return cb(null, { updated, errors, affectedRows, hasErrors: errors.length > 0 });
            }
            default:
...
function buildBPB(options) {
    const multierror = (options && options.multierror !== undefined)
        ? (options.multierror ? 1 : 0)
        : 1;
    const bufSize = (options && options.bufferSize) || (16 * 1024 * 1024);

    // BPB format: version(1B) + items(tag(1B) + length(2B LE) + value(length B LE))
    const bytes = [
        0x01, // version byte
        // TAG_MULTIERROR: continue execution on error (1=yes, 0=stop at first)
        Const.BATCH_TAG_MULTIERROR, 0x04, 0x00,
        multierror & 0xFF, 0x00, 0x00, 0x00,
        // TAG_RECORD_COUNTS: return per-record affected row counts
        Const.BATCH_TAG_RECORD_COUNTS, 0x04, 0x00,
        0x01, 0x00, 0x00, 0x00,
        // TAG_BUFFER_BYTES_SIZE: maximum batch buffer size (default 16 MB)
        Const.BATCH_TAG_BUFFER_BYTES_SIZE, 0x04, 0x00,
        bufSize & 0xFF, (bufSize >> 8) & 0xFF, (bufSize >> 16) & 0xFF, (bufSize >> 24) & 0xFF,
    ];
    return Buffer.from(bytes);
}

function encodeBatchField(msg, value, meta) {
    ...
    switch (meta.type) {
        case Const.SQL_SHORT:
        case Const.SQL_LONG:
            msg.addInt(isNull ? 0 : Math.trunc(value));
            msg.addInt(isNull ? 1 : 0);
            break;

        case Const.SQL_FLOAT:
            msg.addFloat(isNull ? 0 : value);
            msg.addInt(isNull ? 1 : 0);
            break;

        case Const.SQL_DOUBLE:
        case Const.SQL_D_FLOAT:
            msg.addDouble(isNull ? 0 : value);
            msg.addInt(isNull ? 1 : 0);
            break;

        case Const.SQL_INT64:
            msg.addInt64(isNull ? 0 : value);
            msg.addInt(isNull ? 1 : 0);
            break;

        case Const.SQL_INT128:
            msg.addInt128(isNull ? 0 : value);
            msg.addInt(isNull ? 1 : 0);
            break;

        case Const.SQL_BOOLEAN:
            msg.addInt(isNull ? 0 : (value ? 1 : 0));
            msg.addInt(isNull ? 1 : 0);
            break;
        ...
        case Const.SQL_TIME_TZ_EX: {
            if (!isNull) {
                ...
                const t = (ts % TimeCoeff) * 10;
                msg.addUInt(t < 0 ? 0 : t);
                msg.addInt(0); // timezone
                msg.addInt(0); // ext_offset
            } else {
                msg.addUInt(0);
                msg.addInt(0);
                msg.addInt(0);
        ...
        case Const.SQL_ARRAY:
        case Const.SQL_QUAD:
            // value is an OID {high, low} returned by prepareBatchBlobs
            msg.addInt(!isNull && value ? value.high : 0);
            msg.addInt(!isNull && value ? value.low : 0);
            msg.addInt(isNull ? 1 : 0);
            break;
        ...
        case Const.SQL_NULL: {
            ...
            const maxLen = meta.length || 0;
            const fixedBuf = Buffer.alloc(maxLen, 0x20);
            if (!isNull) {
                const b = Buffer.from(value.toString(), Const.DEFAULT_ENCODING);
                b.copy(fixedBuf, 0, 0, Math.min(b.length, maxLen));
            }
            msg.addBuffer(fixedBuf);
        ...
        case Const.SQL_VARYING: {
            // VARCHAR: 2-byte length prefix + data padded to declared max length
            const maxLen = meta.length || 0;
            const varBuf = Buffer.alloc(2 + maxLen, 0);
            if (!isNull) {
                const b = Buffer.from(value.toString(), Const.DEFAULT_ENCODING);
                const actualLen = Math.min(b.length, maxLen);
                varBuf.writeUInt16BE(actualLen, 0);
                b.copy(varBuf, 2, 0, actualLen);
            }
            msg.addBuffer(varBuf);
        ...
        case Const.SQL_DEC16:
            msg.addDecFloat16(isNull ? 0 : value);
            msg.addInt(isNull ? 1 : 0);
            break;

        case Const.SQL_DEC34:
            msg.addDecFloat34(isNull ? 0 : value);
            msg.addInt(isNull ? 1 : 0);
            break;

        default:
            msg.addInt(isNull ? 0 : (value | 0));
            msg.addInt(isNull ? 1 : 0);
    }
}

function prepareBatchBlobs(cnx, transaction, paramsArray, input, callback) {
    const result = new Array(paramsArray.length);

    function processRow(rowIdx) {
        if (rowIdx >= paramsArray.length) {
            return callback(null, result);
        }

        const params = paramsArray[rowIdx];
        const rowValues = new Array(input.length);

        function processParam(paramIdx) {
            if (paramIdx >= input.length) {
                result[rowIdx] = rowValues;
                return processRow(rowIdx + 1);
            }

            const value = paramIdx < params.length ? params[paramIdx] : null;
            const meta = input[paramIdx];

            if (value !== null && value !== undefined && meta.type === Const.SQL_BLOB) {
                cnx.createBlob2(transaction, function(err, blob) {
                    if (err) return callback(err);
                    ...
                    if (Buffer.isBuffer(value)) {
                        ...
                        buf = Buffer.from(value, Const.DEFAULT_ENCODING);
                    } else if (value && value.readable) {
                        const chunks = [];
                        value.on('data', chunk => chunks.push(chunk));
                        value.on('end', () => {
                            buf = Buffer.concat(chunks);
                            uploadBlobForBatch(cnx, blob, buf, (err, oid) => {
                                if (err) return callback(err);
                                rowValues[paramIdx] = oid;
                                processParam(paramIdx + 1);
                            });
                        });
                        value.on('error', callback);
                        return;
                    } else {
                        buf = Buffer.from(JSON.stringify(value), Const.DEFAULT_ENCODING);
                    }
...
function uploadBlobForBatch(cnx, blob, buffer, callback) {
    const chunkSize = cnx.options.blobChunkSize || 1024;
    bufferReader(buffer, chunkSize, (chunk, next) => {
        cnx.batchSegments(blob, chunk, next);
    }, () => {
        cnx.closeBlob(blob, (err) => {
            if (err) return callback(err);
            callback(null, blob.oid);
        }, false);
    });
}
...
module.exports.buildBPB = buildBPB;
module.exports.encodeBatchField = encodeBatchField;
