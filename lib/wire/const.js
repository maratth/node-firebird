...
// DSQL operations
    ...
    // Batch API opcodes (protocol 16+, Firebird 4.0)
    op_batch_create           : 112, // Create batch for a prepared statement
    op_batch_msg              : 113, // Add messages (parameter sets) to batch
    op_batch_exec             : 114, // Execute the batch
    op_batch_rls              : 115, // Release the batch
    op_batch_cs               : 116, // Cancel batch
    op_batch_regblob          : 117, // Register a blob in batch
    op_batch_blob_stream      : 118, // Blob stream data for batch
    op_batch_set_bpb          : 119, // Set blob parameters for batch
    op_batch_cs_response      : 120, // Batch completion status response
};
...
/*************************************/
/* Batch Parameters Block (BPB) tags */
/*************************************/
const batchTag = {
    BATCH_TAG_MULTIERROR        : 1, // Continue execution on error (0=stop, 1=continue)
    BATCH_TAG_RECORD_COUNTS     : 2, // Return per-record update counts
    BATCH_TAG_BUFFER_BYTES_SIZE : 3, // Buffer size in bytes (default 16MB)
    BATCH_TAG_BLOBS_NONE        : 4, // Blobs disabled mode
    BATCH_TAG_BLOB_ID           : 5, // Blob ID handling mode
    BATCH_TAG_DETAILED_ERRORS   : 6, // Max count of detailed error records
    BATCH_TAG_INLINE_BLOBS      : 7, // Inline blobs in messages
};

const batchStatus = {
    BATCH_EXECUTE_FAILED  : -1, // Record failed to execute
    BATCH_EXECUTE_UNKNOWN : -2, // Record execution status unknown
};
